...
**EXPLAIN capture — two modes:**

*Auto-capture (default, tied to monitoring):*
- Triggered automatically when a query on the Query Optimizer leaderboard crosses a flag threshold (e.g. mean time regression >30%, or a sequential scan detected on a large table)
- Runs `EXPLAIN (ANALYZE, BUFFERS)` against a live sample of the flagged query on a throttled schedule (e.g. once per hour per query fingerprint, not every occurrence) to limit load on the customer's database
- Plan Map populates automatically — the user clicks a flagged row on the leaderboard and lands directly on the visualized plan, no manual step required

*Manual paste (ad-hoc):*
- A "Paste plan" mode for pasting raw JSON `EXPLAIN` output directly — covers queries not yet on the leaderboard, queries run in staging/local environments, or a plan a user wants to check before deploying
- Same Map View / List View / Raw JSON rendering and root-cause rules engine apply to pasted plans as to auto-captured ones — the analysis layer doesn't care where the plan came from
- No live database connection required for this mode — useful for sharing a plan with a teammate or checking a query outside the monitored environment

Both modes render through the same plan visualization and rules engine described above — the only difference is how the JSON plan gets in.
